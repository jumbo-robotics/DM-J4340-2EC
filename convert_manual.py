#!/usr/bin/env python3
"""
Manual PDF to Markdown Converter
Converts the DM-J4340-2EC manual PDF to markdown format.
"""
...
def detect_heading(text):
    ...
    if text_upper == text.strip() and len(text.strip()) < 100:
        return True
    ...
    # Check for common heading keywords in Chinese
    heading_keywords = ['规格', '参数', '技术', '尺寸', '说明', '要求', '标准', '型号', '名称', 
                       '安装', '使用', '维护', '故障', '安全', '警告', '注意', '章节', '目录']
...
def extract_manual_content(pdf_path):
    """Extract content from manual PDF with improved formatting"""
    markdown_content = []
    
    try:
        with pdfplumber.open(pdf_path) as pdf:
            total_pages = len(pdf.pages)
            markdown_content.append(f"# DM-J4340-2EC 减速电机说明书\n\n")
            markdown_content.append(f"*版本: V1.0*\n")
...
def convert_manual(pdf_path, output_path=None):
    """Convert manual PDF to markdown file"""
    ...
    # Extract content
    markdown_content = extract_manual_content(pdf_path)
...
if __name__ == "__main__":
    pdf_path = "说明书/DM-J4340-2EC减速电机说明书V1.0.pdf"
    output_path = "DM-J4340-2EC减速电机说明书_formatted.md"
    
    success = convert_manual(pdf_path, output_path)
    if success:
        print("Manual conversion completed successfully!")
    else:
        print("Manual conversion failed!") 